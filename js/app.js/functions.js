$(function () {
  ...
  $('body').on('click', "[data-editable='true']", function (e) {
    ...
    var title =
      type === 'image' ? 'Upload new image' : 'Edit text for this section'
    var tag = $(this).prop('tagName').toLowerCase()
    tag =
      $(this).parents('p').html() === undefined && !$(this).hasClass('non_ck')
        ? tag
        : 'p'
    ...
    var form =
      ...
      (type === 'image'
        ...
        : section_id.indexOf('_background') === -1 &&
          section_id.indexOf('_color') === -1
        ? `<textarea required id="editor"   rows='5'  name='section_title' class='form-control'>` +
          (tag === 'p' ? $(this).text() : $(this).html()) +
          `</textarea>`
        : `<input required type="color" name='section_title' value="` +
          $(this).text() +
          `" class='form-control'>`) +
      `
        </div>
    ...
    if (type !== 'image' && tag !== 'p') {
      setTimeout(() => {
        ...
        editor.on('instanceReady', function (event) {
          if (
            event.editor.getCommand('maximize').state == CKEDITOR.TRISTATE_OFF
          ) {
...
// toggling dashboard hamburger
function toggle_nav () {
  var s_navbar = document.getElementById('dashboard-side-nav')
  var child = document.getElementById('child')

  var _width = s_navbar.offsetWidth
  if (_width > 100) {
    s_navbar.style.width = '50px'
    child.style.display = 'none'
  } else {
    s_navbar.style.width = '200px'
...
function loadJS (
  ...
  if (document.querySelector('[src="' + FILE_URL + '"]') !== null) {
    eval(callback)
    return
  }
  let scriptEle = document.createElement('script')
  scriptEle.setAttribute('src', FILE_URL)
  for (let attribute_index in attributes) {
    scriptEle.setAttribute(attribute_index, attributes[attribute_index])
  }

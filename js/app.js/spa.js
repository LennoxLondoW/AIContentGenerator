    ...
    function (responseText, textStatus, XMLHttpRequest) {
      if (textStatus == 'error') {
        load_to.html(
          `<div style="width:100%; height:` +
            h +
            `; display:flex; align-items:center; justify-content:center;"><svg version="1.1" id="Layer_1"
	xmlns="http://www.w3.org/2000/svg"
      ...
      (entries, observer) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            ...
            loadData((load_to = $(div)), (load_link = url), false)
            observer.disconnect()
          }
        })
      },
...
// custom what happens when objects move to intersecton
function customIntersection (selectors, callback) {
...
$(function () {
  ...
  $('body').on('click', 'a', function (event) {
    ...
    if (
      !$(this).hasClass('non_spa') &&
      replaced.substr(0, 1) !== '#' &&
      $('[data-editable]:first').attr('data-editable') === undefined
    ) {
      event.preventDefault()
      window.history.pushState('view', 'view', href)
      var to = $(this).attr('data-load_to')
      var id = to === undefined ? '#main_field' : to
      loadData((load_to = $(id)), (load_link = href), (relocating_to = id))
      return false
    }
  })
              ...
              $(img)
                ...
                .append(
                  `
                    ...
                    `' alt='` +
                    alt +
                    `' onload='$(this).parent().children().fadeToggle(100)'>
                            `
                )

  ...
  if ($response->isSuccessful()) {
    ...
    if ($this_user->database_error && !$this_user->is_table($this_user->activeTable)) {
      ...
      if ($migration->migrate(false) === "success") {
        ...
        $body = "Please contact admin for further information.<br> Your payment id is " . $payment_id . ".";
      }
    } elseif (count($data) === 0) {
      $gate = true;
    } else {
      $head = 'Transaction Error';
      $body = "Please contact admin for further information.";
    }
    ...
    if ($gate) {
      ...
      if ($save === "success") {
        $body = "The transaction id is: " . $payment_id . ".";
    ...
    $head = 'Transaction Error';
    $body = "Your transaction could not be verified.  " . $body =  $response->getMessage();
  }
} else {
  $head = 'Transaction is declined';
  $body = "Your previous transaction has been rejected by paypal.";
}
...
<body class="fadeBg">
  ...
  <div class="white-bg">
    ...
    <div class="container starterPosition">
      <div class="text"><?php echo $head; ?></div>
    </div>
  </div>
  ...
  <div class="message starterPosition">
    <div class="info">
      ...
      <div><br><br><a style="background:#09c; color:#fff; padding: 15px; border-radius: 5px" href="../../my_account">Go Back</a></div>
    </div>
  </div>

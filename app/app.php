...
class App
{
  ...
  */
  ...
  {
    //(isset($_SERVER['ORIGIN'])? $_SERVER['ORIGIN'] : "");
    if (
      !isset($_POST['csrf_token']) ||
      $_POST['csrf_token'] !== $_SESSION['csrf_token'] ||
      (isset($_POST['honey_pot']) && !empty($_POST['honey_pot'])) ||
      !$this->same_origin() //limitss sending requests from another server to this server
    ) {
  ...
  //login functions
  public function same_origin()
  {
    $origin = isset($_SERVER['HTTP_ORIGIN']) ? $_SERVER['HTTP_ORIGIN'] : " - ";
    return $_SERVER['HTTP_HOST'] === parse_url($origin)['host'];
  }
    ...
    foreach ($rows as $row) {
      $string .= '
		<button class="main_div">
		  ...
		  <p>' . $row['email'] . '</p>
      <p>
		  <form action="' . base_path . str_replace(".php", "", basename($_SERVER['PHP_SELF'])) . '" method="post" enctype="multipart/form-data" class="ajax user_submit">
			<input type="hidden" value="' . csrf_token . '" name="csrf_token">
			<input type="hidden" value="' . $row['id'] . '" name="id">
			<input style="text-transform:capitalize;" class="button" type="submit" name="edit" value="' . ($row['verify'] === "verified" ? "verified" : "Not Verified") . '" data-help_text="Submit your details here.">
		  </form>
		  </p>
		  <p>
      ...
      <div onclick="if(confirm(`Delete this user?`)){$(this).parents(`form`).trigger(`submit`);}" class="button"  style="width:90%!important; margin-top: 20px; border-radius: 5px; padding:8px;">Delete</div>
		  </form>
		  </p>
